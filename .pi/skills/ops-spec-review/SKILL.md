...
Runs **after** `/opsx-propose` and **before** `/opsx-apply`.
...
3. If missing → stop with not found
...
- Optional: user may state a max-rounds override for this run; otherwise use effective config
...
- Edit **only** change artifacts (proposal, design, specs, tasks).
...
- Auto-review follow-up uses this **same full loop** (edits allowed); users may set `OPENSPEC_OPS_AUTO_REVIEW=off`.
