...
name: ops-doctor
description: >
  Run openspec-ops doctor for worktree health (inventory, stale dirs,
  missing paths). Use when worktrees look messy, cleanup is unclear,
  or the user asks to doctor/audit openspec-ops workspaces.
license: MIT
...
- Optional: `--repo <path>` if user is not in the target repo.
